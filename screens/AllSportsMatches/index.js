...
const DATA = [
  {
    ...
    game: 'Cricket',
    task: '( 2 )',
    // img:require("../../assets/images/CricketBall/CricketBall.png")
  },
  {
...
const AllSportsMatches = ({navigation}) => {
  ...
  const fetchSelectedGame = async data => {
    ...
    axios({
      method: 'get',
      // url: `https://api.b365api.com/v1/bet365/?sport_id=${gameType.sportId}&token=${APIKEY}`,
      // url: 'https://api.b365api.com/v1/bet365/',
  ...
  const onSelectTournament = item => {
    ...
    tempArr.forEach(subItem => {
      subItem.league.id === item.league.id
        ? (resultArr.push(subItem), console.log('subItem ----- ', subItem))
        : null;
    });
